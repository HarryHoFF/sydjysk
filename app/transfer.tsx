...
type TransferMode = 'account' | 'iban';

export default function TransferScreen() {
  const router = useRouter();
  const [step, setStep] = useState<TransferStep>('form');
  const [mode, setMode] = useState<TransferMode>('account');
  ...
  const handleContinue = () => {
    setError(null);
    if (!name.trim()) {
      setError('Indtast modtagerens navn');
      return;
    }
    if (mode === 'account') {
      if (!regNr.trim()) {
    ...
    return (
      <View style={styles.container}>
        <View style={styles.successContainer}>
          ...
          <Text style={styles.successTitle}>Overførsel modtaget</Text>
          <Text style={styles.successMessage}>
            Din overførsel på {amount} DKK til {name} er modtaget.
            {'\n\n'}
            Opgaven udføres inden for 1-2 hverdage.
          </Text>
          <View style={styles.successDetails}>
            <DetailRow label="Modtager" value={name} />
            {mode === 'account' ? (
              <>
    ...
    return (
      <View style={styles.container}>
        <View style={styles.topBar}>
          ...
          <Text style={styles.topBarTitle}>Bekræft overførsel</Text>
        ...
        <ScrollView contentContainerStyle={styles.confirmContent} showsVerticalScrollIndicator={false}>
          <View style={styles.confirmCard}>
            ...
            <Text style={styles.confirmTo}>til {name}</Text>
          </View>
          ...
          <Text style={styles.confirmNote}>
            Overførslen udføres inden for 1-2 hverdage.
          </Text>

          <TouchableOpacity style={styles.sendButton} onPress={handleSend} activeOpacity={0.7}>
            <Send size={20} color={colors.white} strokeWidth={1.8} />
            <Text style={styles.sendButtonText}>Bekræft og send</Text>
          </TouchableOpacity>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topBarTitle}>Ny overførsel</Text>
        <View style={styles.backButton} />
      </View>
      ...
      <KeyboardAvoidingView
        ...
        <ScrollView
          ...
          <Text style={[styles.sectionLabel, { marginTop: 28 }]}>Modtager</Text>
